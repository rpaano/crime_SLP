	...
	if ($_SERVER['REQUEST_METHOD'] == "POST"){
		...
		if ($dates != NULL and $brgycasenumber != NULL and $complainant != NULL and $respondent != NULL and $natureofcase != NULL and $disposition_remarks != NULL){
			...
			if (($status != "Closed Case" or $disposition_remarks != "Pending")and($status != "Pending Case" or $disposition_remarks != "Dismissed") and ($status != "Pending Case" or $disposition_remarks != "Withdrawn")){
				...
				$query = mysqli_query($connect, "insert into admin (date_time, brgycasenumber, complainant, respondent, natureofcase, disposition_remarks) values ('$dates','$brgycasenumber','$complainant','$respondent','$natureofcase','$disposition_remarks')") or die ("could not Add");

				$last_id = $connect->insert_id;
				//echo $last_id;

				$query1 = mysqli_query($connect, "insert into pending_cases (status, admin_id) values ('$status','$last_id')") or die ("could not search");

				echo "<script type='text/javascript'>alert('Succesfully added!!'); window.location.replace('insert.php');</script>";
			}
			else
			{
...
<body>
	...
	<div class="topnav">
	  ...
	  <a class="active" href="insert.php">Insert Data</a>
...
<!--	<label for="fname">Nature of Case</label>
	    ...
	    <textarea type="text" id="lname" name="disposition_remarks" placeholder="" ><?=$disposition_remarks?></textarea>
-->
